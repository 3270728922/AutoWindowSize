...
> Minecraft 1.20.1 / Forge 纯客户端模组 — 自动窗口大小、居中、最小尺寸锁定、固定窗口大小、无边框模式、三档窗口置顶、窗口状态循环、5个快捷键、60个分辨率预设，以及启动自动全屏/最大化/无边框
...
2. 将 `autowindowsize-1.1.1.jar` 放入 `.minecraft/mods/` 文件夹。
...
- **配置版本字段**：新增 `[meta]` 分组下的 `configVersion` 选项（仅内部使用，请勿修改），配置界面支持20种语言显示。
- **修复 Issue 模板死链接**："常见问题"链接现在指向实际存在的 `#notes` 章节。
- **README 平台说明**：明确标注 macOS / Linux 为实验性支持。
- **修正作者信息**：所有地方（mods.toml 描述、关于页面）的作者名统一为 `JujuMLQwQ, ML`。
- **更新邮箱**：联系邮箱改为 `jujuawa@qq.com`。
