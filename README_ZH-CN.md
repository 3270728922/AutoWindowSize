...
> **📮 关于源码的说明：由于作者是 GitHub / Git 新手，早期在仓库更新过程中出现过操作失误，曾导致仓库文件一度混乱。如果你下载的源码与实际发布的 jar 对不上，请联系作者邮箱 **3270728922@qq.com** 获取真正的源码。**
...
- **苦力怕论坛（KLBBS）**: [AutoWindowSize](https://klpbbs.com/forum.php?mod=viewthread&tid=173769&fromuid=1116571)
...
- **窗口设置界面**：独立的设置界面，可手动开关最小尺寸锁定 / 固定窗口大小、一键把窗口在所在显示器上居中，实时显示屏幕分辨率和窗口分辨率（全屏时黄色并标注"全屏"），按钮状态实时同步
- **居中窗口按钮**：窗口化时一键把窗口在当前显示器上居中；全屏、最大化、最小化时自动禁用，避免无效操作
- **客户端指令**：`/aws toggle`、`/aws lock`、`/aws unlock`、`/aws status`、`/aws gui`、`/aws center`、`/aws fixed`，游戏内聊天栏直接操作
- **全屏自动禁用**：进入全屏时临时禁用最小尺寸锁定与固定窗口大小，退出全屏后按全屏前状态自动恢复
- **自定义按键**：在控制设置中绑定按键打开窗口设置界面（默认未绑定）
- **分辨率过低自动禁用**：当配置分辨率或硬编码最小分辨率高于玩家屏幕分辨率时，仅自动禁用最小尺寸锁定且不改动窗口大小；固定窗口大小与居中不受影响（它们不依赖配置值）
...
| `/aws fixed` | 切换固定窗口大小：开启后窗口锁定为当前大小，无法再更改（不能放大/缩小，最大化按钮禁用） |

最小尺寸锁定被禁用时（分辨率过低或全屏），相关指令会显示红色提示说明原因。固定窗口大小只受全屏和最大化禁用，不受分辨率过低影响。
...
| 固定窗口大小按钮 | 点击把当前窗口大小完全锁死；最大化/全屏时灰色不可点击 |
