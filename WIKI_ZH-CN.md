...
- **启动自动设置窗口大小**：启动游戏后延迟 1.5 秒自动将窗口设置为配置的分辨率并居中，避免加载界面拉伸。
...
- **窗口设置界面**：独立的设置界面，可手动开关最小尺寸锁定 / 固定窗口大小 / 下次启动自动全屏、一键居中，实时显示屏幕分辨率和窗口分辨率（全屏时黄色并标注"全屏"），按钮状态实时同步。
...
| 下次启动自动全屏按钮 | 切换持久偏好，不影响当前窗口；与自动最大化互斥 |
| 下次启动自动最大化按钮 | 切换持久偏好，不影响当前窗口；与自动全屏互斥 |
| 居中窗口按钮 | 一键把窗口在当前显示器居中 |
| 比例切换按钮 | 循环切换 16:9 / 16:10 / 4:3 / 5:4 / 21:9 / 自定义，按钮上实时显示当前比例和窗口分辨率 |
| 分辨率预设按钮 | 按当前比例分组，每行 4 个，点一下立即切换到该分辨率并居中；已匹配当前分辨率的预设和超过屏幕的预设自动禁用 |
| 自定义分辨率输入框 | 切到"自定义"时显示，可手动输入宽和高，点应用立即切换 |
| 完成按钮 | 返回视频设置界面 |

### 分辨率预设

- 预设按比例分组：
  - **16:9**：856×482 / 1280×720 / 1366×768 / 1600×900 / 1920×1080 / 2560×1440
  - **16:10**：1024×640 / 1280×800 / 1440×900 / 1680×1050 / 1920×1200
  - **4:3**：800×600 / 1024×768 / 1280×960 / 1400×1050 / 1600×1200
  - **5:4**：1280×1024 / 1600×1280 / 1920×1536
...
- 输入值自动限制在 856~屏幕宽度、482~屏幕高度范围内。
- 手动拖动窗口后，如果当前分辨率不匹配任何比例预设，比例会自动切到"自定义"。

### 实时刷新机制

- 按钮状态**实时同步**，不需要重开界面。
- 拖动窗口边缘时，分辨率数值保持不动并显示橙色提示。
- 停止拖动满 **0.2 秒**后，橙色提示熄灭，同时分辨率刷新为当前窗口大小。
...
```toml
[window]
# width / height：启动窗口尺寸，也是最小尺寸锁定的下限。
# width 范围 856 ~ 7680，默认 1280；height 范围 482 ~ 4320，默认 720。
...
> 注释采用"上英文、下中文"的双语写法，每个配置项上方留空行、`[startup]` 内分"作者项 / 玩家项"两组；`Range` 由 Forge 自动生成，不重复书写。

### 注意事项

- **升级本模组时请先手动删除旧配置文件** `.minecraft/config/autowindowsize-client.toml`。由于部分原因，新版对配置注释和项做了调整，直接覆盖 jar 而不删旧文件时，Forge 不会重写注释、也可能缺少新增项；删除后启动游戏会自动生成全新配置。
