...
- **分辨率预设**：8种比例（16:9 / 16:10 / 4:3 / 5:4 / 21:9）每种12个常用分辨率一键切换，外加自定义宽高输入（带实时边界校验）。
- **最小尺寸锁定**：开启后窗口只能放大，不能缩小到配置尺寸以下，保证 UI 布局不被破坏。
- **固定窗口大小**：开启后把窗口**当前大小**完全锁死——不会跳变到配置分辨率、也不会强制居中，只是不能再改大小。标题栏最大化按钮会在系统层被禁用（GLFW_RESIZABLE=false），从根上杜绝"假最大化"；窗口最大化或全屏时固定窗口大小开关本身也会禁用。全屏仍然可用，退出全屏后回到固定尺寸。
- **居中窗口按钮**：窗口化时一键把窗口在当前显示器上居中；全屏、最大化、最小化、已居中时自动禁用并提示原因。
- **无边框模式**：去除窗口标题栏和边框（GLFW_DECORATED=false）。支持窗口化、最大化和全屏（伪全屏）三种状态。可设置启动自动无边框。
- **窗口置顶（3种模式）**：关闭、普通（GLFW 悬浮）、强制（每帧通过 glfwFocusWindow + 焦点丢失回调重新抢焦点，可覆盖教室监控软件等其他置顶窗口）。
- **循环切换窗口状态**：一个按钮/快捷键即可在 窗口化 → 最大化 → 全屏 之间循环切换。
- **启动自动全屏 / 自动最大化 / 自动无边框**：玩家可在窗口设置里持久勾选"下次启动自动全屏"、"自动最大化"或"自动无边框"；整合包作者也可在配置里设一次性引导。
- **窗口位置记忆**：可选择退出时保存窗口位置与大小，下次启动恢复（而非强制居中）；与自动全屏/最大化兼容。
- **辅助功能设置原生入口**：在「选项 → 辅助功能设置」中原生插入「窗口管理与分辨率设置」按钮（从视频设置迁移，兼容 Embeddium 等替换视频设置界面的模组）。
- **窗口设置界面**：独立的设置界面，可手动开关最小尺寸锁定 / 固定窗口大小 / 启动自动全屏 / 启动自动最大化 / 启动自动无边框 / 无边框 / 窗口置顶 / 位置记忆 / 调试，一键居中，循环切换状态，分辨率预设与自定义输入，关于页面，四行实时信息区（屏幕分辨率 / 窗口状态+配置文件状态 / 状态详细说明 / 输入边界提示），按钮状态实时同步。
- **5个快捷键**（默认未绑定）：打开设置、居中、切换无边框、循环状态、切换置顶。全部带聊天反馈。
- **关于页面**：游戏内信息页面，包含9个模块（制作目的、MOD介绍、使用方法、指令、快捷键、下载渠道、反馈渠道、注意事项、作者信息），支持可点击链接和邮箱复制，支持20种语言。
- **调试模式**：将详细诊断信息（预设可用性检查、无边框状态切换、窗口状态变化等）输出到游戏日志。
- **客户端指令**：共17个顶级指令（含 `borderless auto` 子指令共18个） — `/aws help`（分页，3页）、`gui`、`about`、`info`、`version`、`config`、`status`、`toggle`、`fixed`、`center`、`fullscreen`、`maximize`、`remember`、`top`、`debug`、`borderless`（含 `auto` 子指令）、`resolution`，全部切换指令支持 `true`/`false` 参数，`/aws top` 支持状态名参数，游戏内聊天栏直接操作。
...
无边框模式通过设置 `GLFW_DECORATED = false` 去除窗口标题栏和边框。支持三种窗口状态：

| 状态 | 行为 |
|------|------|
| **窗口化无边框** | 简单实现 — 仅去除装饰。窗口保持当前大小。注意：窗口无法再通过标题栏拖动，请配合"记住窗口状态"或设置分辨率来移动窗口。 |
| **最大化无边框** | 最大化时去除装饰。窗口填满屏幕工作区（不含任务栏）。 |
| **全屏无边框（伪全屏）** | 以全屏分辨率运行的无边框窗口。与真正的全屏（独占模式）不同，伪全屏允许快速 Alt+Tab 切换，不会干扰其他显示器。 |

**启动自动无边框**：`autoBorderless` 配置项（默认关闭），开启后下次启动自动进入无边框模式。通过 `/aws borderless auto` 或设置界面切换。

**已知限制**：
- GLFW 只提供一个 `GLFW_DECORATED` 总开关 — 无法只去除标题栏而保留可调整大小的边框（这需要平台相关的 Win32 API 调用）。
- 窗口化无边框采用简单实现（仅去除装饰）。在窗口化状态下切换无边框时保持窗口总大小不变比较复杂，目前暂不实现。

---

## 窗口置顶

三种模式，由 `alwaysOnTopMode` 配置项控制（0–2）：

| 模式 | 实现 | 适用场景 |
|------|------|----------|
| **0 — 关闭** | 窗口行为正常。 | 默认。 |
| **1 — 普通** | 设置 `GLFW_FLOATING = true`。窗口保持在其他非悬浮窗口之上，但可能被其他悬浮窗口覆盖。 | 一般使用。 |
| **2 — 强制** | 每帧调用 `glfwFocusWindow(hwnd)`，外加焦点丢失回调（窗口失去焦点时重新抢焦点）。可覆盖其他置顶窗口（如教室监控软件、 kiosk 应用）。 | 绝对需要窗口在最前面的场景。 |

**性能说明**：强制模式每帧调用 `glfwFocusWindow`，有少量 CPU 开销（仅在极高帧率如 1000+ FPS 时可测量）。在正常帧率（60–240 FPS）下影响可忽略不计。
...
|--------|------|----------|
| 打开窗口设置 | 打开设置界面 | — |
| 居中窗口 | 在当前显示器上居中窗口 | "窗口已居中" / "已经居中了" / 不可用时提示原因 |
| 切换无边框 | 切换无边框模式开/关 | "无边框模式已开启" / "无边框模式已关闭" |
| 循环切换窗口状态 | 在 窗口化 → 最大化 → 全屏 之间循环 | "窗口状态：[状态]" |
| 切换窗口置顶 | 循环切换 关闭 → 普通 → 强制 | "窗口置顶：[模式]" |

---

## 指令

所有指令以 `/aws` 开头（客户端指令，仅游戏内可用）。输入 `/aws help [页码]` 查看分页列表（共3页，每页8条）。
...
| `/aws about` | 打开关于页面 |
| `/aws info` | 显示详细的窗口、配置和分辨率信息（6行精简输出） |
| `/aws version` | 显示模组版本、作者、许可证和联系信息 |
| `/aws config` | 在文件资源管理器中打开配置文件夹 |
...
| `/aws center` | 把窗口在当前显示器居中（不可用时提示原因） |
| `/aws fullscreen [true/false]` | 切换全屏模式；可加 `true`/`false` |
| `/aws maximize [true/false]` | 切换最大化模式；可加 `true`/`false` |
| `/aws remember [true/false]` | 切换窗口位置记忆；可加 `true`/`false` |
| `/aws top [off/normal/force]` | 循环切换窗口置顶模式，或直接指定状态名 |
| `/aws debug [true/false]` | 切换调试日志；可加 `true`/`false` |
| `/aws borderless [true/false]` | 切换无边框模式；可加 `true`/`false` |
| `/aws borderless auto [true/false]` | 切换下次启动自动无边框；可加 `true`/`false` |
| `/aws resolution [比例] <预设>` | 按比例和预设设置分辨率并居中（如 `/aws resolution 16:9 1920x1080`），Tab 自动补全 |

最小尺寸锁定被禁用时（分辨率过低或全屏），相关指令会显示红色提示说明原因。`/aws resolution` 指令会校验输入是否超过屏幕分辨率，超出范围的值会自动钳制到最近的合法值。比例和预设参数支持 Tab 自动补全。
...
1. 主菜单：`选项...` → `辅助功能设置...` → 点击 `窗口管理与分辨率设置` 按钮。
2. 游戏中：`Esc` → `选项...` → `辅助功能设置...` → 点击 `窗口管理与分辨率设置` 按钮。
3. 按下绑定的键位（默认未绑定）。
4. 聊天栏输入 `/aws gui`。
...
- **配置自动迁移（v1.1.2起）**：升级时模组会在首次启动自动检测旧配置版本并迁移，无需手动删除配置文件夹，进入世界后聊天框会提示迁移完成。如果是从 1.0.9 之前版本升级，旧的 `autowindowsize-client.toml` 会被检测到并在日志中警告（可手动删除）。
...
**Q：`/aws` 指令有什么用？**
A：`/aws toggle` 切换最小尺寸锁定（可加 `true`/`false` 明确开启/关闭），`/aws status` 查看当前窗口状态和锁定状态，`/aws info` 查看详细窗口信息，`/aws gui` 打开设置界面，`/aws center` 把窗口居中，`/aws fixed` 切换固定窗口大小。输入 `/aws help` 查看完整指令列表。
