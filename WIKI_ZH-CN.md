...
- **客户端指令**：共15个 — `/aws help`（分页）、`gui`、`status`、`toggle`、`lock`、`unlock`、`fixed`、`center`、`fullscreen`、`maximize`、`remember`、`top`、`debug`、`borderless`（含 `auto` 子指令）、`resolution`，游戏内聊天栏直接操作。
...
所有指令以 `/aws` 开头（客户端指令，仅游戏内可用）。输入 `/aws help [页码]` 查看分页列表（共2页，每页8条）。

| 指令 | 说明 |
|------|------|
| `/aws help [页码]` | 显示指令列表（分页，每页8条） |
| `/aws gui` | 打开窗口设置界面 |
...
| `/aws debug` | 切换调试日志 |
| `/aws borderless` | 切换无边框模式 |
| `/aws borderless auto` | 切换下次启动自动无边框 |
...
- **配置自动迁移（v1.1.1起）**：升级时模组会在首次启动自动检测旧配置版本并迁移，无需手动删除配置文件夹，进入世界后聊天框会提示迁移完成。如果是从 1.0.9 之前版本升级，旧的 `autowindowsize-client.toml` 会被检测到并在日志中警告（可手动删除）。
