...
public class Config {
    ...
    // 配置版本号：用于自动迁移。1.1.0及之前没有此字段（视为1）；1.1.1起为2。
    // 不要手动修改。启动时若检测到旧版本，会自动把缺失字段写回配置文件。
    ...
    static {
        ...
        CONFIG_VERSION = builder
                .comment(
                        ...
                        "Internal version for automatic config migration.",
                        "Default 1 = legacy config (pre-1.1.1). On first load, the mod",
                        "detects this, migrates, and sets it to the current version.",
                        "If this is lower than the latest, the mod will automatically",
                        "fill in any missing config fields and update this number.")
                .translation("config.autowindowsize.meta.configVersion")
